...
The llm will "see" the images generated by matplotlib (requires a multimodal
model, obviously), so it can interpret them. They are also displayed
in the Execution Summary.

It can be disabled with the valve ENABLE_IMAGE_GENERATION (disables sending to
the model, but not rendering).

Generated images aren't cleaned up.
##

This is an openwebui tool that can run arbitrary python.
...
class Tools:
    class Valves(BaseModel):
        ...
        DOCKER_IMAGE: str = Field(
            ...
            # default="pythonds",            description="docker image to run",
        )
